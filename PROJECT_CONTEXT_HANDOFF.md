...
- Existing planner links also expose a platform-superadmin-only diagnostic action at `GET /api/v1/planner/links/{id}/eligibility-diagnostics/`, making the same safe evidence available directly from the Media Proposals workspace.
...
- `/sales/proposals` now shows platform admins per-link eligible/published/excluded counts plus a Diagnostics button that opens pipeline counts, exclusions, migration status, backend identity, and safe sample rows for the ESPA units under investigation.

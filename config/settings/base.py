...
MIDDLEWARE = [
    ...
    "apps.observability.middleware.OperationalModeMiddleware",
...
MEDIA_STORAGE_PROVIDER = get_env("MEDIA_STORAGE_PROVIDER", "r2").strip().lower()
...
CLOUDINARY_UPLOAD_PRESET = get_env("CLOUDINARY_UPLOAD_PRESET", "omms_inventory_signed")
CLOUDINARY_ROOT_FOLDER = get_env("CLOUDINARY_ROOT_FOLDER", "omms")
if MEDIA_STORAGE_PROVIDER not in {"r2", "s3", "cloudinary"}:
    raise ImproperlyConfigured("MEDIA_STORAGE_PROVIDER must be one of: r2, s3, cloudinary.")
if MEDIA_STORAGE_PROVIDER == "cloudinary" and (not CLOUDINARY_URL or not CLOUDINARY_UPLOAD_PRESET):
    raise ImproperlyConfigured("MEDIA_STORAGE_PROVIDER=cloudinary requires CLOUDINARY_URL and CLOUDINARY_UPLOAD_PRESET.")
...
ACCESS_TOKEN_LIFETIME_MINUTES = get_int("ACCESS_TOKEN_LIFETIME_MINUTES", 20)
SESSION_INACTIVITY_TIMEOUT_HOURS = get_int("SESSION_INACTIVITY_TIMEOUT_HOURS", 72)
REFRESH_COOKIE_MAX_AGE_SECONDS = get_int(
    "REFRESH_COOKIE_MAX_AGE_SECONDS",
    SESSION_INACTIVITY_TIMEOUT_HOURS * 60 * 60,
)
AUTH_REFRESH_COOKIE_NAME = get_env("AUTH_REFRESH_COOKIE_NAME", "omms_refresh_session")
AUTH_REFRESH_COOKIE_DOMAIN = get_env("AUTH_REFRESH_COOKIE_DOMAIN", "")
AUTH_REFRESH_COOKIE_SAMESITE = get_env("AUTH_REFRESH_COOKIE_SAMESITE", "None" if not DEBUG else "Lax")
AUTH_REFRESH_COOKIE_SECURE = get_bool("AUTH_REFRESH_COOKIE_SECURE", not DEBUG)
AUTH_USER_MODEL = "users.User"
TEST_RUNNER = "core.test_runner.BackendOnlyDiscoverRunner"

REST_FRAMEWORK = {
    "DEFAULT_AUTHENTICATION_CLASSES": [
        "apps.users.authentication.RollingJWTAuthentication",
